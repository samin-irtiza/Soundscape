...
ffmpegopts = {
    'before_options': '-nostdin -seekable 1 -reconnect 1 -reconnect_streamed 1 -reconnect_at_eof 1 -reconnect_delay_max 5',
...
class Music(commands.Cog):
    ...
    async def connect_(self, ctx, *, channel: discord.VoiceChannel=None):
        ...
        if not channel:
            try:
                ...
                embed = discord.Embed(title="", description="No channel to join. Please call `,join` from a voice channel.", color=discord.Color.dark_blue())
    ...
    async def play_(self, ctx, *, search: str=None):
        ...
        if not player.msgswitch:
          embed = discord.Embed(title="", description=f"Queued [{source['title']}]({source['webpage_url']}) [{ctx.author.mention}]", color=discord.Color.dark_blue())
          await ctx.send(embed=embed)
    ...
    async def pause_(self, ctx):
        ...
        if not vc or not vc.is_playing():
            embed = discord.Embed(title="", description="I am currently not playing anything", color=discord.Color.dark_blue())
    ...
    async def resume_(self, ctx):
        ...
        if not vc or not vc.is_connected():
            embed = discord.Embed(title="", description="I'm not connected to a voice channel", color=discord.Color.dark_blue())
            return await ctx.send(embed=embed)
    ...
    async def remove_(self, ctx, pos : int=None):
        ...
        if pos == None:
            ...
            try:
                ...
                embed = discord.Embed(title="", description=f"Removed [{s['title']}]({s['webpage_url']}) [{s['requester'].mention}]", color=discord.Color.dark_blue())
                await ctx.send(embed=embed)
            except:
    ...
    async def queue_info(self, ctx):
        ...
        if player.queue.empty():
            embed = discord.Embed(title="", description="Queue is empty", color=discord.Color.dark_blue())
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(title=f'Queue for {ctx.guild.name}', description=fmt, color=discord.Color.dark_blue())
    ...
    async def now_playing_(self, ctx):
        ...
        embed = discord.Embed(title="", description=f"[{vc.source.title}]({vc.source.web_url}) [{vc.source.requester.mention}] | `{duration}`", color=discord.Color.dark_blue())
    ...
    async def change_volume(self, ctx, *, vol: float=None):
        ...
        if not vc or not vc.is_connected():
            embed = discord.Embed(title="", description="I am not currently connected to voice", color=discord.Color.dark_blue())
            return await ctx.send(embed=embed)
        
        if not vol:
            embed = discord.Embed(title="", description=f"🔊 **{(vc.source.volume)*100}%**", color=discord.Color.dark_blue())
            return await ctx.send(embed=embed)

        if not 0 < vol < 101:
            embed = discord.Embed(title="", description="Please enter a value between 1 and 100", color=discord.Color.dark_blue())
            return await ctx.send(embed=embed)
        ...
        embed = discord.Embed(title="", description=f'**`{ctx.author}`** set the volume to **{vol}%**', color=discord.Color.dark_blue())
        await ctx.send(embed=embed)
    ...
    async def repeat(self, ctx, state=None):
      ...
      embed = discord.Embed(title="", description="Repeat is now set to **{}**".format(player.repeat),color=discord.Color.dark_blue())
      await ctx.send(embed = embed)
      return
    ...
    async def search(self,ctx,*,search):
      ...
      embed = discord.Embed(title="**Top Search Results**", description=fmtstr,color=discord.Color.dark_blue())
      embed.set_footer(text=f"{ctx.author.display_name}", icon_url=ctx.author.avatar_url)
      await ctx.send(embed = embed)
      try:
